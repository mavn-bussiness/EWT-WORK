<x-app-layout>
    ...
    <style>
        ...
        .aws-pagination a:hover {
            background-color: rgba(255, 255, 255, 0.1);
        }

        .aws-pagination .current {
            background-color: var(--aws-orange);
            color: var(--aws-navy);
        }
    </style>
    ...
    <div class="aws-content">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('teacher.assignments.create') }}" class="aws-btn bg-[var(--aws-orange)] text-[var(--aws-navy)] hover:bg-[var(--aws-orange-hover)]">
            ...
            <div class="aws-card overflow-x-auto">
                <table class="aws-table min-w-full">
                    ...
                    <tr>
                        ...
                        <th class="text-xs text-[var(--aws-text-light)]">Max Score</th>
                    ...
                    @foreach ($assignments as $assignment)
                        <tr>
                            ...
                            <td class="text-sm text-[var(--aws-text-light)]">{{ $assignment->max_score }}</td>
                            <td class="text-sm text-[var(--aws-text-light)] space-x-2">
                                <a href="{{ route('teacher.assignments.edit', $assignment->id) }}" class="text-[var(--aws-blue)] hover:text-[var(--aws-blue)]/80">Edit</a>
                                <a href="{{ route('teacher.assignments.grade', $assignment->id) }}" class="text-[var(--aws-green)] hover:text-[var(--aws-green)]/80">Grade</a>
                                <form action="{{ route('teacher.assignments.destroy', $assignment->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-[var(--aws-red)] hover:text-[var(--aws-red)]/80" onclick="return confirm('Are you sure?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
