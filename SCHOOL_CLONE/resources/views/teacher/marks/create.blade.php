<x-app-layout>
    ...
    <div class="aws-sidebar">
        ...
        <nav class="space-y-2">
            ...
            <a href="{{ route('teacher.marks.index') }}" class="active">
                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="aws-card p-6">
                <h3 class="text-xl font-semibold text-white mb-6 flex items-center">
                    ...
                    Record Marks: {{ $assessment->title }}
                </h3>

                <form action="{{ route('teacher.marks.store', $assessment->id) }}" method="POST">
                    @csrf

                    <div class="mb-4">
                        <p class="text-[var(--aws-text-light)]"><strong>Class:</strong> {{ $assessment->class->name }}{{ $assessment->class->stream ? ' - ' . $assessment->class->stream : '' }}</p>
                        <p class="text-[var(--aws-text-light)]"><strong>Subject:</strong> {{ $assessment->subject->name }}</p>
                        <p class="text-[var(--aws-text-light)]"><strong>Max Score:</strong> {{ $assessment->max_score }}</p>
                        <p class="text-[var(--aws-text-light)]"><strong>Date:</strong> {{ $assessment->assessment_date->format('Y-m-d') }}</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="aws-table min-w-full">
                            <thead>
                            <tr>
                                <th class="text-xs text-[var(--aws-text-light)]">Student</th>
                                <th class="text-xs text-[var(--aws-text-light)]">Score</th>
                                <th class="text-xs text-[var(--aws-text-light)]">Grade</th>
                                <th class="text-xs text-[var(--aws-text-light)]">Remarks</th>
                            </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm text-[var(--aws-text-light)]">
                                        <input type="number" name="scores[{{ $student->id }}]"
                                               ...
                                               class="aws-input w-20"
                                               min="0" max="{{ $assessment->max_score }}"
                                               step="1">
                                    </td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $student->marks->first()?->grade ?? '-' }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">
                                        <input type="text" name="remarks[{{ $student->id }}]"
                                               value="{{ $student->marks->first()?->teacher_remarks ?? '' }}"
                                               class="aws-input w-full">
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
