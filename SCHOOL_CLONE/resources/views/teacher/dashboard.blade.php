<x-app-layout>
    ...
    <div class="aws-content">
        ...
        <div class="max-w-7xl mx-auto">
            @if (isset($error))
                <div class="aws-alert aws-alert-error mb-6">
                    {{ $error }}
                </div>
            @else
                <!-- Classes Overview -->
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($classSubjects as $classSubject)
                            <div class="aws-card p-4">
                                <h4 class="text-lg font-medium text-white">{{ $classSubject->class->name }}{{ $classSubject->class->stream ? ' - ' . $classSubject->class->stream : '' }}</h4>
                                <p class="text-[var(--aws-text-light)] opacity-75">{{ $classSubject->subject->name }}</p>
                                <a href="{{ route('teacher.classes.show', $classSubject->class->id) }}" class="text-[var(--aws-orange)] hover:text-[var(--aws-orange-hover)] mt-2 block">View Details →</a>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="aws-card overflow-x-auto">
                        <table class="aws-table min-w-full">
                            ...
                            <tr>
                                ...
                                <th class="text-xs text-[var(--aws-text-light)]">Subject</th>
                                <th class="text-xs text-[var(--aws-text-light)]">Due Date</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach ($assignments as $assignment)
                                <tr>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assignment->title }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assignment->class->name }}{{ $assignment->class->stream ? ' - ' . $assignment->class->stream : '' }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assignment->subject->name }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assignment->due_date->format('Y-m-d') }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="aws-card overflow-x-auto">
                        <table class="aws-table min-w-full">
                            ...
                            @foreach ($assessments as $assessment)
                                <tr>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assessment->title }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assessment->class->name }}{{ $assessment->class->stream ? ' - ' . $assessment->class->stream : '' }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assessment->subject->name }}</td>
                                    <td class="text-sm text-[var(--aws-text-light)]">{{ $assessment->assessment_date->format('Y-m-d') }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                ...
                @if ($attendancePending)
                    <div class="aws-alert aws-alert-warning">
