            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($classes as $class)
                    <div class="aws-card p-4">
                        <h4 class="text-lg font-medium text-white">{{ $class->name }}{{ $class->stream ? ' - ' . $class->stream : '' }}</h4>
                        <a href="{{ route('teacher.attendance.mark', $class->id) }}" class="text-[var(--aws-orange)] hover:text-[var(--aws-orange-hover)] mt-2 block">
                            Mark Attendance →
                        </a>
                    </div>
                @endforeach
