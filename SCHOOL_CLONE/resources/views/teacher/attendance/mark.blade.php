<x-app-layout>
    ...
    <div class="aws-sidebar">
        ...
        <nav class="space-y-2">
            ...
            <a href="{{ route('teacher.attendance.index') }}" class="active">
                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="aws-card p-6">
                <h3 class="text-xl font-semibold text-white mb-6 flex items-center">
                    ...
                    Mark Attendance: {{ $class->name }}{{ $class->stream ? ' - ' . $class->stream : '' }}
                </h3>
                ...
                <form action="{{ route('teacher.attendance.store', $class->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <p class="text-[var(--aws-text-light)]"><strong>Date:</strong> {{ now()->format('Y-m-d') }}</p>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm text-[var(--aws-text-light)]">
                                        <div class="aws-select-wrapper">
                                            <select name="statuses[{{ $student->id }}]" class="aws-select">
                                                <option value="">Select</option>
                                                <option value="present" {{ $student->attendances->first()?->status == 'present' ? 'selected' : '' }}>Present</option>
                                                <option value="absent" {{ $student->attendances->first()?->status == 'absent' ? 'selected' : '' }}>Absent</option>
                                                <option value="late" {{ $student->attendances->first()?->status == 'late' ? 'selected' : '' }}>Late</option>
                                            </select>
                                        </div>
                                    </td>
                                    <td class="text-sm text-[var(--aws-text-light)]">
                                        <input type="text" name="remarks[{{ $student->id }}]"
                                               value="{{ $student->attendances->first()?->status ? ($student->attendances->first()->remarks ?? '') : '' }}"
