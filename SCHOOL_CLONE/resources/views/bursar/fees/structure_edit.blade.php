```blade
<x-app-layout title="Edit Fee Structure">
    <div class="container-fluid px-4 py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="mb-1 h3 fw-bold">Edit Fee Structure</h1>
                <p class="text-muted mb-0">Update fee structure for {{ $feeStructure->class_level }}</p>
            </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('bursar.fees.structure.update', $feeStructure->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="term_id" id="term_id" class="form-select @error('term_id') is-invalid @enderror" required>
                                ...
                                @foreach($terms as $term)
                                    <option value="{{ $term->id }}" {{ $term->id == $feeStructure->term_id ? 'selected' : '' }}>
                                        {{ $term->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="class_level" id="class_level"
                                   ...
                                   value="{{ old('class_level', $feeStructure->class_level) }}" required>
                            @error('class_level')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" name="tuition" id="tuition" step="0.01"
                                   ...
                                   value="{{ old('tuition', $feeStructure->tuition) }}" required>
                            @error('tuition')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" name="boarding" id="boarding" step="0.01"
                                   ...
                                   value="{{ old('boarding', $feeStructure->boarding) }}">
                            @error('boarding')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" name="development" id="development" step="0.01"
                                   ...
                                   value="{{ old('development', $feeStructure->development) }}">
                            @error('development')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
