                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.events.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" required rows="4">{{ old('description') }}</textarea>
                        </div>

                        <div class="mb-4">
                            <x-label for="event_date" :value="__('Event Date')" class="required" />
                            <x-input id="event_date" type="date" name="event_date" :value="old('event_date')" class="block mt-1 w-full" required />
                        </div>

                        <div class="mb-4">
                            <x-label for="start_time" :value="__('Start Time')" />
                            <x-input id="start_time" type="time" name="start_time" :value="old('start_time')" class="block mt-1 w-full" />
                        </div>

                        <div class="mb-4">
                            <x-label for="end_time" :value="__('End Time')" />
                            <x-input id="end_time" type="time" name="end_time" :value="old('end_time')" class="block mt-1 w-full" />
                        </div>

                        <div class="mb-4">
                            <x-label for="location" :value="__('Location')" />
                            <x-input id="location" type="text" name="location" :value="old('location')" class="block mt-1 w-full" />
                        </div>
