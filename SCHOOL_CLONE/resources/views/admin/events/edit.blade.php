    ...
    <div class="aws-content">
        ...
        <div class="aws-card">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('admin.events.update', $schoolEvent->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-label for="title" :value="__('Title')" class="aws-label required" />
                        <x-input id="title" type="text" name="title" :value="old('title', $schoolEvent->title)" class="aws-input" required autofocus />
                    </div>

                    <div>
                        <x-label for="description" :value="__('Description')" class="aws-label required" />
                        <textarea id="description" name="description" class="aws-textarea" required rows="4">{{ old('description', $schoolEvent->description) }}</textarea>
                    </div>

                    <div>
                        <x-label for="event_date" :value="__('Event Date')" class="aws-label required" />
                        <x-input id="event_date" type="date" name="event_date" :value="old('event_date', \Carbon\Carbon::parse($schoolEvent->event_date)->format('Y-m-d'))" class="aws-input" required />
                    </div>

                    <div>
                        <x-label for="start_time" :value="__('Start Time')" class="aws-label" />
                        <x-input id="start_time" type="time" name="start_time" :value="old('start_time', $schoolEvent->start_time ? \Carbon\Carbon::parse($schoolEvent->start_time)->format('H:i') : '')" class="aws-input" />
                    </div>

                    <div>
                        <x-label for="end_time" :value="__('End Time')" class="aws-label" />
                        <x-input id="end_time" type="time" name="end_time" :value="old('end_time', $schoolEvent->end_time ? \Carbon\Carbon::parse($schoolEvent->end_time)->format('H:i') : '')" class="aws-input" />
                    </div>

                    <div>
                        <x-label for="location" :value="__('Location')" class="aws-label" />
                        <x-input id="location" type="text" name="location" :value="old('location', $schoolEvent->location)" class="aws-input" />
                    </div>
